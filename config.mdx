...
When set, Wavelet matches the JWT token's claims against this column to filter diffs per-client. See [JWT auth](/server/jwt-auth) for details.
...
See [Postgres CDC sources](/sources/postgres-cdc) for prerequisites and setup.
...
Provide either `secret` or `jwksUrl`, not both. See [JWT auth](/server/jwt-auth) for the full guide.
