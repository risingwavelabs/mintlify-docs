...
Wavelet uses JWT tokens to filter real-time view diffs per client. A single materialized view can serve multiple tenants, and each client only sees rows that match their token claims.
