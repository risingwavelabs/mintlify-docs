...
Wavelet uses JWT tokens to filter real-time query diffs per client. A single materialized view can serve multiple tenants, and each client only sees rows that match their token claims.

## How it works

1. You define a query with a `filterBy` column
...
- **One cursor, many clients.** Wavelet creates a single RisingWave subscription cursor per query. All clients share it. Filtering happens in-memory on the server after fetching from the cursor.
...
- **No filter = all rows.** If a query does not have `filterBy`, all subscribers receive all diffs regardless of JWT claims.
...
- **HTTP API.** JWT filtering currently applies only to WebSocket subscriptions. HTTP GET requests to queries return unfiltered results (or you can filter manually via query parameters).
