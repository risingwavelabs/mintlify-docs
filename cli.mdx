...
```json
[
  { "type": "create", "resource": "event", "name": "game_events" },
  { "type": "create", "resource": "query", "name": "leaderboard" },
...
- **Row type interfaces** for each query (e.g., `LeaderboardRow`)
- **Event type interfaces** for each event (e.g., `GameEventsEvent`)
- **`TypedWaveletClient`** class with typed `.queries` and `.events` accessors
- **`QueryName`** literal union type
- **`useWavelet` overloads** for React with per-query return types
...
<Tip>
Add `columns` to your query definitions for reliable offline codegen without needing a running RisingWave.
</Tip>
