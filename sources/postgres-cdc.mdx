...
Wavelet can ingest data from existing PostgreSQL databases via CDC (Change Data Capture). Changes in your Postgres tables (inserts, updates, deletes) automatically flow into RisingWave, where they can be referenced in query definitions.
...
Use these names when referencing CDC tables in query definitions.
...
## Combining CDC sources with events

You can combine CDC data from existing databases with Wavelet events (HTTP writes) in the same query:
