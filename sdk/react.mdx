...
description: "useWavelet React hook: subscribe to real-time query data with automatic state management, diff merging, and loading/error states."
---

Wavelet provides a React hook that fetches the initial state of a query and subscribes to real-time updates. State is managed internally; your component re-renders when data changes.
...
|-----------|------|----------|-------------|
| `queryName` | `string` | Yes | Name of the query to subscribe to |
...
**Always use `keyBy` when your query has a natural primary key.** This is more efficient and avoids edge cases with JSON comparison.
...
1. **Initial fetch**. `useWavelet` calls `query.get(params)` via HTTP to load the current snapshot
...
The hook cleans up the WebSocket subscription when the component unmounts or when `queryName`/`keyBy`/`params` change.
...
Subscribe to multiple queries in the same component:
