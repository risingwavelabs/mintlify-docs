...
description: "WaveletClient API reference: connect to Wavelet, subscribe to real-time query diffs, query data, and emit events from TypeScript."
---
...
Returns a `QueryHandle` with `get()` and `subscribe()` methods.

```typescript
const leaderboard = client.query('leaderboard')
```

### `query.get(params?)`

Fetch the current state of a query via HTTP GET.

```typescript
// All rows
const rows = await client.query('leaderboard').get()

// With query param filters
const rows = await client.query('leaderboard').get({ player_id: 'alice' })
```

Returns `Promise<T[]>` where `T` defaults to `Record<string, unknown>`.

**Errors:**
- `QUERY_NOT_FOUND`: the query does not exist
- `AUTH_ERROR`: invalid or missing token (when JWT is configured)
- `SERVER_ERROR`: other server errors

### `query.subscribe(handlers)`

Subscribe to real-time diffs via WebSocket. Returns an `unsubscribe` function.

```typescript
const unsub = client.query('leaderboard').subscribe({
  onData: (diff) => {
...
**Auth rejection (code 4000):** If the server rejects the connection (invalid token, query not found), the client does **not** reconnect. It calls `onError` instead.
...
Returns an `EventHandle` with `emit()` and `emitBatch()` methods.

```typescript
const events = client.event('game_events')
```
...
```typescript
await client.event('game_events').emit({
  player_id: 'alice',
  action: 'win',
  score: 100,
})
...
### `event.emitBatch(data)`

Write multiple events in one request via HTTP POST.

```typescript
await client.event('game_events').emitBatch([
...
// Typed query. rows are LeaderboardRow[]
const rows = await client.query<LeaderboardRow>('leaderboard').get()

// Typed event. emit is type-checked
await client.event<GameEventsEvent>('game_events').emit({
  player_id: 'alice',
  action: 'win',
  score: 100,
})
...
client.query<LeaderboardRow>('leaderboard').subscribe({
  onData: (diff) => {
...
// Autocomplete works. queries.leaderboard, events.game_events
const rows = await client.queries.leaderboard.get()
await client.events.game_events.emit({ player_id: 'alice', action: 'win', score: 100 })
```
